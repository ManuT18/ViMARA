...
### 2.2. Análisis de Viabilidad y Migración a WebApp (El Salto a la Web)
...
- **Accesibilidad y "Cero Fricción" (Zero-Friction):** En el ámbito académico y profesional, obligar a un usuario (alumno, profesor o cliente) a descargar una aplicación de más de 100MB desde una tienda virtual solo para visualizar una maqueta genera una barrera de entrada muy alta. La migración a una WebApp garantiza un acceso instantáneo y universal mediante un simple enlace web o escaneando un código QR.
- **Definición del Stack Tecnológico Frontend:** Para asegurar que la web no solo fuera funcional sino que tuviera un rendimiento sobresaliente, se decidió descartar tecnologías web antiguas. El análisis concluyó en la adopción de **React** (específicamente su versión 19) como biblioteca principal para la construcción de la interfaz, orquestado mediante **Vite** para garantizar tiempos de carga y compilación ultrarrápidos. 
- **Librerías de Renderizado y Visualización:** Para el renderizado de gráficos tridimensionales en el navegador, se analizó y aprobó el uso de **Three.js** (el estándar indiscutido de la industria para WebGL) en combinación con el componente web **`<model-viewer>`** impulsado por Google. Esto permitía, en teoría, renderizar mallas complejas directamente en el DOM del navegador de forma eficiente.
- **Complejidad de la Migración de Interfaz (UI):** Se determinó que reconstruir la interfaz gráfica (menús, botones, selectores) desde el sistema nativo "Canvas" de Unity hacia componentes web puros (HTML/CSS/JS) era un esfuerzo altamente justificado. La web ofrece un diseño adaptable (*responsive*) de forma natural (Mobile-First). Esto permitió a ViMARA adoptar una estética limpia, ligera y visualmente muy superior ("Premium") a la que habitualmente se logra exportando interfaces gráficas directamente desde el motor de videojuegos.

### 2.3. Evolución del Tracking AR y la Integración de Zapworks
Si bien el nuevo ecosistema web (React + Three.js) resolvía de manera excelente la interfaz de usuario y la visualización de los modelos 3D en pantalla, la ejecución de la Realidad Aumentada pura dentro del navegador (WebAR) presentaba su propio desafío técnico.

En un principio, se evaluaron bibliotecas open-source como MindAR.js o AR.js. Aunque ofrecían una puerta de entrada gratuita, su rendimiento de seguimiento (*tracking*), la estabilidad de la cámara y la capacidad de procesar mallas 3D complejas en celulares de gama media presentaban deficiencias notorias y fluctuaciones (jittering) en los modelos.

Tras un análisis exhaustivo y consultas académicas, se evaluó el ecosistema comercial **Zapworks** y su **Universal AR SDK**. Esta tecnología resultó ser el punto de inflexión necesario para el proyecto. La empresa Zappar desarrolló motores de visión por computadora (*Computer Vision*) altamente optimizados en C++ y los compiló a WebAssembly (Wasm). Esto permite que tareas sumamente pesadas, como el seguimiento de imágenes (apuntar la cámara a un plano impreso) o la detección de superficies físicas (mesas/suelos), funcionen dentro del navegador web con una estabilidad, anclaje y fluidez que compiten directamente con las aplicaciones nativas, eliminando las limitaciones del WebAR tradicional.
